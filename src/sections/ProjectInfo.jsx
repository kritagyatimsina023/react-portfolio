  ...
  useGSAP(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const run = () => {
        ...
        // gsap.set(".pi-tag", { opacity: 1 });
        // Tags + hero description
        if (tagsRef.current) {
          const tags = tagsRef.current.querySelectorAll(".pi-tag");
        ...
        // --- ABOUT section: SplitText on headings + paragraphs ---
        ...
        const cards = Array.from(highlightsGridRef.current?.children || []);
        ...
        return () => splits.forEach((s) => s.revert && s.revert());
      };
