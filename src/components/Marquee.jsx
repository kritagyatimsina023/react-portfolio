...
const Marquee = ({
  ...
  useEffect(() => {
    ...
    const raf = requestAnimationFrame(() => {
      const totalWidth = track.scrollWidth / 2; // half because we duplicate items

      // Reset position
      gsap.set(track, { x: 0, force3D: true });

      const duration = totalWidth / 80; // pixels per second → ~80px/s
      ...
      observerRef.current = Observer.create({
        onChangeY(self) {
          ...
          gsap
            ...
            .to(tl, { timeScale: factor * 3, duration: 0.15, overwrite: true })
            .to(tl, { timeScale: reverse ? -1 : 1, duration: 0.8 }, "+=0.2");
        },
      });
    });
    ...
    >
      ...
      >
        {allItems.map((text, index) => (
          <span
            key={index}
            className="flex items-center px-16 gap-x-8 marquee-text-responsive"
          >
            {text}
            <span className={`inline-block mx-4 ${iconClassName}`}>✦</span>
          </span>
        ))}
      </div>
    </div>
